...
So far, we have told Ruckig the number of DoFs as a C++ template parameter. If you don't know the number of DoFs at compile-time, you can also set the template parameter to zero and pass the DoFs as an argument in the constructor:
...
However, we recommend to use the template parameter when possible: First, it is a few percent faster. Second, it is convenient for real-time programming due to its easier handling of memory allocations. When using the dynamic degrees of freedom, make sure to allocate the memory of all vectors beforehand.
